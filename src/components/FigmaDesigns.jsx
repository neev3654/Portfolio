...
export default function FigmaDesigns() {
  ...
  return (
    <>
      <ChapterSection id="figma-designs" className="bg-bg text-text">
        <div ref={containerRef} className="mx-auto flex h-full max-w-7xl items-center w-full relative pt-24 md:pt-12">
          
          {/* Background bloom — consistent with other sections */}
          <div className="absolute right-1/4 bottom-1/4 w-[500px] h-[500px] bg-accent-purple/5 rounded-full blur-[100px] pointer-events-none" />

          <div className="w-full flex flex-col pt-12 md:pt-20 relative z-10 font-sans">
            
            {/* HEADER */}
            <div className="max-w-4xl px-4 md:px-0 mb-12 md:mb-16">
              <p className="mb-4 text-xs font-semibold tracking-widest uppercase text-muted">
                Chapter 04
              </p>
              <div className="font-display text-4xl sm:text-6xl md:text-7xl lg:text-[6.5rem] font-medium leading-[1] tracking-tightest mb-6">
                <MaskedTextReveal text="UI/UX" as="span" /> <span className="text-gradient"><MaskedTextReveal text="Designs" as="span" delay={0.2} /></span>
              </div>
              <p className="text-muted text-lg md:text-xl max-w-2xl leading-relaxed mt-6">
                A showcase of my user interface concepts and product design case studies,
                focusing on aesthetics, usability, and design systems.
              </p>
            </div>

            {/* DESIGN GRID */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-4 md:px-0 pb-20">
              {figmaDesigns.map((design) => (
                <div key={design.id} className="group relative rounded-[24px] bg-card overflow-hidden card-hover-lift flex flex-col border border-border/40 gpu-accelerated shadow-sm">
                  {/* Image Container with Hover Overlay */}
                  <div className="relative w-full aspect-[4/3] overflow-hidden bg-border/10">
                    <img
                      src={design.image}
                      alt={design.title}
                    ...
                    <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out flex flex-col items-center justify-center p-6 text-center z-10 backdrop-blur-sm">
                      ...
                      <div className="flex items-center gap-4 translate-y-6 group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] delay-75">
                        <button
                          ...
                          className="flex items-center justify-center w-12 h-12 bg-white/20 hover:bg-white/30 text-white rounded-full backdrop-blur-md transition-colors shadow-lg"
                        ...
                        {design.figmaLink && (
                          <a
                            ...
                            className="flex items-center gap-2 px-6 py-3 bg-accent-purple hover:bg-[#8e45f5] text-white font-medium rounded-full transition-colors shadow-xl"
                          >
                        ...
                        {design.caseStudyLink && design.caseStudyLink !== "#" && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 px-6 py-3 bg-card text-text hover:bg-border/30 font-medium rounded-full transition-colors shadow-xl"
                          >
